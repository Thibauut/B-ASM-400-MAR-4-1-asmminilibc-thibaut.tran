...
for:
    mov r12b, [rdi] ; stocke le pointeur vers le premier caractère de rdi dans r12
    cmp r12b, 0 ; si le caractère est 0, on sort de la boucle car on a atteint la fin de la chaîne
    je endFor ; on sort de la boucle
    mov rsi, r9 ; on remet rsi à sa valeur initiale
for2:
    mov r13b, [rsi] ; stocke le pointeur vers le premier caractère de rsi dans r13
...
endFor2:
    ...
    jmp for ; on recommence la boucle for pour le prochain caractère
endFor:
    ret
